...
total_runs=$((n_runs * ${#algslist[@]} * ${#readslist[@]} * ${#targetslist[@]}))
...
for alg in "${algslist[@]}"; do # ----------------------------------- ALG
  ...
  mkdir -p ~/engage/logs/server/"${exp_name}"/"${alg}"
  ...
  for run in $(seq "$start_run" $((n_runs + start_run - 1))); do
    ...
    for reads_per in "${readslist[@]}"; do # ---------------------------  READS_PER
      ...
      for target in "${targetslist[@]}"; do # -------------------- TARGET
        echo -e "$GREEN -- -- -- -- -- -- -- -- STARTING TARGET $NC$target"
        echo -e "$GREEN -- -- -- -- -- -- -- -- - $NC$exp_path/$target"

        rm -r ~/engage/logs/client/"${exp_path}"/"${target}"_*
        rm -r ~/engage/logs/server/"${exp_path}"/"${target}"_*
        ...
        for server_node in "${server_nodes[@]}"; do
          ...
          -DlogFilename=/tmp/cass/${OAR_JOB_ID}/results/${exp_path}/${target}_${server_node} \
        ...
        for client_node in "${client_nodes[@]}"; do
          ...
          > /home/pfouto/engage/logs/client/${exp_path}/${target}_${client_node}" 2>&1 | sed "s/^/[c-$client_node] /" &
          client_pids+=($!)
          i=$((i + 1))
        done
        sleep 90
...
echo "Getting logs"
for server_node in "${server_nodes[@]}"; do
  oarsh "$server_node" "cp -r /tmp/cass/${OAR_JOB_ID}/results/* /home/pfouto/engage/logs/server/" &
done
wait
exit
